...
export const AdminGateScanner: React.FC<Props> = ({ onNotify, getHeaders }) => {
  ...
  // Gate configuration & database list
  const [gates, setGates] = useState<GateItem[]>([]);
  const [gateName, setGateName] = useState('Gate 01 - Main Arch Gate');
  const [operatorName, setOperatorName] = useState('Gate Officer (NASH)');
  const [gateModalOpen, setGateModalOpen] = useState(false);
  const [editingGate, setEditingGate] = useState<Partial<GateItem> | null>(null);
  const [gateSaveLoading, setGateSaveLoading] = useState(false);

  const fetchGates = async () => {
    try {
      const res = await fetch('/api/admin/gates');
      if (res.ok) {
        const data: GateItem[] = await res.json();
        setGates(data || []);
        if (data && data.length > 0) {
          const currentExists = data.some(g => g.gate_name === gateName && g.is_active);
          if (!currentExists) {
            const firstActive = data.find(g => g.is_active) || data[0];
            if (firstActive) setGateName(firstActive.gate_name);
          }
        }
      }
    } catch (err) {
      console.error('Failed to load database gates:', err);
    }
  };
  ...
  const handleSaveGate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingGate?.gate_name || !editingGate?.gate_code) {
      alert('Gate name and gate code are required.');
      return;
    }

    setGateSaveLoading(true);
    try {
      const isNew = !editingGate.id;
      const url = isNew ? '/api/admin/gates' : `/api/admin/gates/${editingGate.id}`;
      ...
      if (res.ok) {
        await fetchGates();
        setEditingGate(null);
        if (onNotify) onNotify(`Gate ${isNew ? 'created' : 'updated'} successfully.`);
      } else {
        const err = await res.json();
        alert(err.message || 'Failed to save gate.');
      }
    } catch (err) {
      console.error('Error saving gate:', err);
    } finally {
      setGateSaveLoading(false);
    }
  };
  ...
  const handleDeleteGate = async (id: string, name: string) => {
    ...
    try {
      ...
      if (res.ok) {
        await fetchGates();
        if (onNotify) onNotify('Gate removed successfully.');
      }
    } catch (err) {
  ...
  const handleToggleGate = async (gate: GateItem) => {
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-200 pb-4">
        ...
        {/* Database-driven Gate Officer Configuration */}
        <div className="flex flex-wrap items-center gap-2 bg-slate-100 p-1.5 rounded-xl border border-slate-200 text-xs">
          <select
            value={gateName}
            onChange={e => setGateName(e.target.value)}
            className="bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-emerald-600 max-w-[220px]"
          >
            {gates.filter(g => g.is_active).map(g => (
              <option key={g.id} value={g.gate_name}>
                {g.gate_name} ({g.gate_code})
              </option>
            ))}
            {gates.filter(g => g.is_active).length === 0 && (
              <option value="Gate 01 - Main Arch Gate">Gate 01 - Main Arch Gate</option>
            )}
          </select>
          <input
            type="text"
            ...
            className="w-32 bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 font-medium text-slate-700 focus:outline-none focus:ring-1 focus:ring-emerald-600"
          />
          <button
            type="button"
            onClick={() => setGateModalOpen(true)}
            className="px-2.5 py-1.5 bg-emerald-800 hover:bg-emerald-700 text-white rounded-lg font-bold flex items-center gap-1.5 cursor-pointer transition shadow-2xs"
            title="Configure Database Gates"
          >
            <Settings className="w-3.5 h-3.5" />
            <span>Manage Gates ({gates.length})</span>
          </button>
        </div>
      </div>
      ...
      {gateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-200">
            <div className="p-5 bg-gradient-to-r from-[#0f4d2a] to-[#135d34] text-white flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center gap-2">
                <Settings className="w-5 h-5 text-amber-300" />
                <h3 className="font-bold text-base">Gate Management (Database-Driven)</h3>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-6 space-y-6">
              {/* Add / Edit Gate Form */}
              <form onSubmit={handleSaveGate} className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-slate-700 flex items-center gap-1.5">
                    <Plus className="w-3.5 h-3.5 text-emerald-700" />
                    {editingGate?.id ? 'Edit Gate' : 'Create New Entrance Gate'}
                  </span>
                  {editingGate && (
                    <button
                      type="button"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. Gate 05 - West Pavilion"
                      value={editingGate?.gate_name || ''}
                      onChange={e => setEditingGate({ ...editingGate, gate_name: e.target.value })}
                      className="w-full px-3 py-2 text-xs bg-white border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-600"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-600 uppercase mb-1">
                    ...
                    <input
                      ...
                      value={editingGate?.gate_code || ''}
                      onChange={e => setEditingGate({ ...editingGate, gate_code: e.target.value.toUpperCase() })}
                      className="w-full px-3 py-2 text-xs bg-white border border-slate-300 rounded-xl font-mono focus:outline-none focus:ring-2 focus:ring-emerald-600 uppercase"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="sm:col-span-2">
                    ...
                    <input
                      type="text"
                      placeholder="e.g. For Batch 1980-2000 alumni & guests"
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingGate?.display_order !== undefined ? editingGate.display_order : gates.length + 1}
                      onChange={e => setEditingGate({ ...editingGate, display_order: parseInt(e.target.value, 10) || 1 })}
                      className="w-full px-3 py-2 text-xs bg-white border border-slate-300 rounded-xl font-mono focus:outline-none focus:ring-2 focus:ring-emerald-600"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <label className="inline-flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={editingGate?.is_active ?? true}
                      onChange={e => setEditingGate({ ...editingGate, is_active: e.target.checked })}
                      className="w-4 h-4 text-emerald-700 rounded focus:ring-emerald-500"
                    />
                    <span className="text-xs font-bold text-slate-700">Gate Active / Open</span>
                  </label>

                  <button
                    type="submit"
                    disabled={gateSaveLoading}
                    className="px-4 py-2 bg-[#0f4d2a] hover:bg-[#135d34] text-white font-bold text-xs rounded-xl shadow-xs cursor-pointer transition disabled:opacity-50"
                  >
                    {gateSaveLoading ? 'Saving...' : editingGate?.id ? 'Update Gate' : 'Add Gate to System'}
                  </button>
                </div>
              </form>

              {/* Existing Gates Table */}
              <div className="space-y-2">
                <span className="text-xs font-bold uppercase text-slate-500">Configured Gates ({gates.length})</span>
                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden divide-y divide-slate-100 text-xs">
                  {gates.map(gate => (
                    <div key={gate.id} className="p-3.5 flex items-center justify-between hover:bg-slate-50 transition">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-slate-900">{gate.gate_name}</span>
                          <span className="px-2 py-0.5 bg-slate-100 font-mono text-[11px] rounded text-slate-600 font-semibold">
                            {gate.gate_code}
                          </span>
                          <span
                            onClick={() => handleToggleGate(gate)}
                            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider cursor-pointer transition ${
                              gate.is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-500'
                            }`}
                          >
                        ...
                        {gate.description && (
                          <p className="text-[11px] text-slate-500">{gate.description}</p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          type="button"
                          onClick={() => handleDeleteGate(gate.id, gate.gate_name)}
                          className="p-1.5 text-slate-500 hover:text-red-700 hover:bg-red-50 rounded-lg cursor-pointer transition"
                          title="Delete Gate"
                        >
                  ...
                  {gates.length === 0 && (
                    <div className="p-6 text-center text-slate-400">
                      No gates registered in the database.
                    </div>
                  )}
                </div>
              </div>
